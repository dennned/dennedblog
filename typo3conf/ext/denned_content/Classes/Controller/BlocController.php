...
class BlocController extends \TYPO3\CMS\Extbase\Mvc\Controller\ActionController {
	...
	public function blocHomePageAction() {
		...
		if($this->settings['mobile'] == 1 && $this->isMobile() == true){
			return false;
		}
		
		if ($this->settings['actus']){
			$data = $this->blocRepository->findByUidNews($this->settings['actus']);
			//\TYPO3\CMS\Core\Utility\DebugUtility::debug($data,'$$$data');
	...
	/*private function getNewsData($actu) {
		...
		$where = 'uid IN ('.$actu.')';
		$where .= $this->cObj->enableFields($from);
