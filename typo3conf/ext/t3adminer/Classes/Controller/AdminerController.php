...
class AdminerController extends \TYPO3\CMS\Backend\Module\BaseScriptClass {
	...
	function main() {
		...
		if (($this->id && $access) || ($GLOBALS['BE_USER']->user['admin'] && !$this->id)) {
			...
			$exportDirectory =
				GeneralUtility::getFileAbsFileName(trim($extensionConfiguration['exportDirectory']));
			if (!is_dir($exportDirectory)) {
				$exportDirectory =
					GeneralUtility::getFileAbsFileName($GLOBALS['TYPO3_CONF_VARS']['BE']['fileadminDir']);
			}
			...
			$this->MCONF['ADM_relative_path'] =
				ExtensionManagementUtility::extRelPath('t3adminer') . $this->MCONF['ADM_subdir'];
