...
class TtNewsSitemapGenerator extends AbstractSitemapGenerator {
	...
	protected function generateSitemapContent() {
		...
		if (count($this->pidList) > 0) {
			...
			$where = " hidden=0 AND deleted=0 AND pid IN (" . implode(",", $this->pidList) . ")".($this->isNewsSitemap ? " AND crdate>=" . (time() - 48*60*60) : '').$languageCondition;
			$limit = $this->limit;
			$groupBy = "datetime DESC";
			$orderBy = "";
	...
	protected function getNewsItemUrl($newsRow, $forceSinglePid = NULL) {
		...
		if ($link == '') {
			$conf = array(
				'additionalParams' => '&tx_news_pi1[news]=' . $newsRow['uid'].'&tx_news_pi1[action]=detail&tx_news_pi1[controller]=News',
				'forceAbsoluteUrl' => 1,
